function [DETCMOD,OmegaMOD,FMOD,DETC_All,AllOmegas,DETC_OmjtkF1]=gbmsre_mac(P,A,B) 
...
%      : Computation may fail even for a very small scale model. Refer to Testing_GB_computation_time_mac.m.
...
%% (1) Installation
%% (1-1) Important: Before installation,
%       - Go to System Preferences and open Security & Privacy
%       - Allow apps downloaded from "Open anyway".
%       These steps are required because otherwise Singular cannot be
%       installed properly.
%% (1-2) Install Singular from the following website.
%       - Go to https://www.singular.uni-kl.de/index.php/singular-download.html
%       - Choose OS X and Choose the option "Installation from a dmg File"
%       - Follow the instruction there. 
%% (1-3) Open Singular. 
%    	- "/Applications" is the folder in which this matlab code
%          writes the Singular procedure code. Singular reads the code and computes
%          and store the solutions as ascii file in this folder, and this matlab code load the file
...
%%     To do so, copy and paste " execute(read("/Applications/GB_singular.ascii")); timer; ns; " 
...
%% [Section 0] : Checking your Singular directory.
        
        home_dir=strcat('/Applications/Singular.app'); 
        addpath(home_dir);
        outputfile="/Applications/GB_singular.ascii";
...
%% [2-2] Writing the code Implementing the GB method in Singular
        ...
        strR='/Applications/Sol_partR.txt'; 
        strC='/Applications/Sol_partC.txt';
...
%% [Section 3] Ask Singular to solve the problem     
        ...
        disp('(1) Open the Singular window.')
