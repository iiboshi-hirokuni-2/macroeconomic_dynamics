...
addpath(genpath('./MODMethod'))
   ...
   for  Panel=1:2 % choose from 1 to 6
         ...
         elseif Panel==2,     theta1=1.5; theta2=0.9;  theta3=0.0593; theta4=0.0593;
                                    x_i=1; y_i=3;                        
    ...
    vn{5}='\phi_B'; vn{6}='\phi_y';  vn{7}='\sigma'; 
        ...
        par_U=[ 2.50     2.50    0.1       0.1       ];   % Upper bound
        disp('    Result with the following parameter values for example')
        disp(vn), disp(par)

%         
         [RC1,RC2]=func_Model_SW_gsys(par);    
       % (1) Area for DET                            
                    if RC1(1,1)==1 && RC1(1,2)==1,                         
                         disp('     (1)  Solution is DET'),disp(RC1)    ;
                   end                    
                % (2) Area for INDET                    
                    if RC1(1,1)==1 && RC1(1,2)==0,                         
                         disp('     (2)  Solution is  INDET'),disp(RC1)   ; 
                          disp(' ');          end
                % (3) Area for NSS                               
                    if RC1(1,1) ==0 ,                         
                         disp('     (3)  Solution is   NSS'),disp(RC1) ;   
                             disp(' ');        end
...
%             EvalModelQ;
              [RC1,RC2]=func_Model_SW_gsys(par_temp);
    
            % For DET 
            if ~isnan(RC1(1,1))
                  DETA1_R1_temp(i)=(RC1(1,1)<1);  DETA2_R1_temp(i)=(RC1(1,2)<1) ;
             else
                DETA1_R1_temp(i)=10; DETA2_R1_temp(i)=10
            end

             if ~isnan(RC2(1,1))
                  DETA1_R2_temp(i)=(RC2(1,1)<1);  DETA2_R2_temp(i)=(RC2(1,2)<1) ;
             else
        ...
        %%
        DETA1= DETA1_R1_temp+DETA1_R2_temp;   DETA2= DETA2_R1_temp+DETA2_R2_temp; 
...
%              colormap(hot(8))
    ...
    pause(0.3);
       %%
   saveas(gca, [ './graph/Region-panel' num2str(Panel)  '_gensys.fig'  ] );
