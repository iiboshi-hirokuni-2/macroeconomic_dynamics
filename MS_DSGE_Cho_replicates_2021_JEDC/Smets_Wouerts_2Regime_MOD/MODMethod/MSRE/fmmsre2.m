function [DETC,FCC,OmegaK,GammaK,FK,OtherOutput,IRS,IRS2]=fmmsre2(P,A,B,C,R,Opt) 
...
%% [4] Path-Wise Impulse-Responses Function under Regime-Switching
if nargout>7  
    D12=diag(vecsig);
    for i=1:S
        FT{i,1}=OmegaK{i};
        GT{i,1}=GammaK{i}*RR{i};
    end
    for t=2:T-1
        for i=1:S
            FT{i,t} =FT{i,t-1}*OmegaK{i};
            GT{i,t} =(GT{i,t-1}+FT{i,t-1}*GammaK{i})*R{i};
        end
    end
