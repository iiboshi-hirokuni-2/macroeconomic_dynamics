...
syms  mc zcap rk k pk c inve y lab pinf w r kp Elab Epinf   Ew Ec Epk Einve ;   %a b g qs ms   spinf sw kpf kp;   
        %ewma epinfma zcapf rkf kf pkf cf  invef yf labf wf rrf
% lag variables
syms  mc1 zcap1 rk1 k pk1 c1 inve1 y1 lab1 pinf1 w1 r1 kp1 Elab1 Epinf1   Ew1 Ec1 Epk1 Einve1 ;
% % forward variables
% syms  mc0 zcap0 rk0 k0 pk0 c0 inve0 y0 lab0 pinf0 w0 r0 kp0;

% exogenous variables
syms a b g qs ms   spinf sw kpf ;  % ea eb eg eqs em epinf ew;  
syms  eta_lab eta_pinf   eta_w eta_c eta_pk eta_inve
...
%% make MATRIX
Y_t = transpose([ mc zcap rk k pk c inve y lab pinf w r kp  Elab Epinf   Ew Ec Epk Einve ]);
  
Y_b = transpose([ mc1 zcap1 rk1 k pk1 c1 inve1 y1 lab1 pinf1 w1 r1  kp1  Elab1 Epinf1   Ew1 Ec1 Epk1 Einve1 ]);

Epsilon_t = transpose([  a b g qs ms   spinf sw ]);

Eta_t = transpose([ eta_lab eta_pinf   eta_w eta_c eta_pk eta_inve ]);
...
%	eq2      [name='FOC capacity utilization, SW Equation (7)']
 ...
 %  eq4       [name='Definition capital services, SW Equation (6)']
  ...
  %   eq5     [name='Investment Euler Equation, SW Equation (3)']
	eq5 =  - inve + (1/(1+cbetabar*cgamma))* (inve1 + cbetabar*cgamma*Einve+(1/(cgamma^2*csadjcost))*pk ) +qs ;
 %   eq6         [name='Arbitrage equation value of capital, SW Equation (4)']
    eq6 =  - pk  -r+Epinf-0*b ...
                + (1/((1-chabb/cgamma)/(csigma*(1+chabb/cgamma))))*b ...
                + (crk/(crk+(1-ctou)))*rk(1) ...
                + ((1-ctou)/(crk+(1-ctou)))*Epk ;
%   eq7	      [name='Consumption Euler Equation, SW Equation (2)']
	eq7 =  -c + (chabb/cgamma)/(1+chabb/cgamma)*c1 ...
                + (1/(1+chabb/cgamma))*Ec ...
                +((csigma-1)*cwhlc/(csigma*(1+chabb/cgamma)))*(lab-Elab ) ...
                - (1-chabb/cgamma)/(csigma*(1+chabb/cgamma))*(r-Epinf + 0*b) +b ;
...
%   eq11	      [name='Wage Phillips Curve, SW Equation (13), with (12) plugged for mu_w']
  eq11=  - w + (1/(1+cbetabar*cgamma))*w1...
   ...
   %   eq13       [name='Law of motion for capital, SW Equation (8) (see header notes)']              
    ...
    %% forecasting error  
     fe1 =   - eta_lab + lab - Elab1;
     fe2 =  -  eta_pinf  +pinf - Epinf1;
     fe3=   - eta_w  + w - Ew1;
     fe4 = -eta_c  + c - Ec1;
     fe5 = -eta_pk + pk -Epk1;
     fe6 = -eta_inve + inve - Einve1;
...
GAM1j = jacobian(System_of_Eq, Y_b);
