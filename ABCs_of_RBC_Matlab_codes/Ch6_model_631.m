...
eq1 = BETTA*(exp(ct)/exp(cf))*(exp(rf)+(1-DELTA)) -1 ; % Euler equation

eq2 = (1- THETA)*(1-exp(ht))*exp(yyt)/exp(ht)- AA*exp(ct);

eq3 = exp(yyt) + (1-DELTA)*exp(kt) - exp(kp)- exp(ct);

eq4 = exp(lamt)*exp(kt)^THETA*exp(ht)^(1-THETA) -exp(yyt) ;

eq5 = THETA*exp(yyt)/exp(kt) - exp(rt);
...
kt=x1(1); yyt=x1(2); ct=x1(3); ht=x1(4); rt =x1(5); lamt=x1(6);
...
title('Impulse response (non linear)','Fontsize',16)
